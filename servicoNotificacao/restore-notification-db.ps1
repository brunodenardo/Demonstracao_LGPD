...
$DB_USER = $env:POSTGRES_USER_NOTIFICATION
$DB_NAME = $env:POSTGRES_DB_NOTIFICATION
$DB_PASSWORD = $env:POSTGRES_PASSWORD_NOTIFICATION
...
$command = @"
docker run --rm --network=demonstracao_lgpd_default -v "$CurrentDir/backups:/backups" -e PGPASSWORD="$DB_PASSWORD" postgres:latest pg_restore --data-only -t usuario -h "$DB_HOST" -U "$DB_USER" -d "$DB_NAME" "/backups/$BackupFile"
"@
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Erro ao restaurar os dados da tabela 'usuario'."
    exit 1
}

# Lógica da Blacklist (após a restauração)
